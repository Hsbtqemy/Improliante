...
| GOU-01 | 🟠 | P1 — le résultat d'une réunion doit être figé avec ses règles | **Réouvert** par l'inventaire ARCH-01 (lot 10). Le lot 4 (`d1ba3a5`) a bien fait le gros : registre électoral complet, règles figées à la clôture, registre non réouvrable. Mais il a figé les RÈGLES d'une réunion close, pas son CONTENU — résolutions, sujets et compte rendu s'y ajoutent encore depuis le back-office (éprouvé). Et « un seul service pour les pouvoirs, quel que soit le chemin » était faux : `PouvoirInline` de l'admin contourne le plafond statutaire |
| ARCH-01 | 🟠 | P1 — les règles centrales restent trop dépendantes des vues | **Inventaire fait** (lot 10) : [`regles-hors-services.md`](regles-hors-services.md), sept points. Relevé à 🟠 : trois d'entre eux pèsent plus que le constat d'origine — un PV régénéré détruit le précédent, une réunion archivée accepte encore du contenu, un devis facturé peut être refacturé. Aucun n'est corrigé, l'arbitrage revient à l'association |
