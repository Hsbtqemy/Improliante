...
- [x] **PUB-01**, la limite : contact, réservations et « mot de passe oublié » refusent au-delà d'un nombre d'envois par origine et par fenêtre, réglable. L'origine ne se LIT pas dans `X-Forwarded-For` — le client l'écrit — mais se compte à partir de la droite, selon `PROXIES_DE_CONFIANCE`. À zéro, l'en-tête est ignoré
- [x] **PUB-01**, la borne du message : le champ de contact a une longueur maximale, réglable. L'audit le relevait comme « pas de longueur métier explicite »
- [ ] **PUB-01**, la jauge : une réservation successive peut encore saturer un événement. Un plafond par adresse a été écrit puis RETIRÉ — deux tests le refusaient, et le dépôt avait tranché en toutes lettres : « bloquer par e-mail donnerait une fausse sécurité (on en change en une seconde) tout en gênant un cas légitime ». La limite de débit rend la saturation lente, pas impossible. La réponse que l'audit suggère est une confirmation de réservation qui EXPIRE si elle n'est pas confirmée — elle demande un envoi de courriel qui fonctionne, donc DEP-1, et c'est un arbitrage de produit avant d'être une tâche
- [x] Le formulaire de « mot de passe oublié » ne permet plus de noyer la boîte d'un membre : il se borne DEUX fois, par origine et par adresse visée, et l'ordre compte — l'origine sort sans toucher au compteur de l'adresse, sinon un attaquant depuis une origine bloquée épuiserait la part de sa victime. Refuser ne change rien à ce que la page affiche, donc n'ouvre pas l'énumération des comptes
...
- [ ] FRONT-08, passe QA : `pilotage/qa/accessibilite-front-08.md` est jouée et cochée par un humain — clavier réel, zoom 200 et 400 %, 375 px, ce qu'un lecteur d'écran annonce sur un formulaire REFUSÉ (état qu'aucun balayage ne visite), et la NETTETÉ des images servies : le `srcset` du lot 17 se mesure dans le HTML, le choix qu'en fait le navigateur ne se mesure pas
